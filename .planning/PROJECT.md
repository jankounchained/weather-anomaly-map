...
- [x] Per-day trend row rebuilt as a split-violin visualization — each of the last 7 days shows a prior-25yr (left) vs recent-5yr (right) hand-rolled Gaussian-KDE distribution on one shared Y-axis, with per-half mean ticks (the gap surfacing the climate-shift signal), the preserved actual-value diamond, a rug fallback when a half has too few years (n<20), and a reviewer-finalized 5-item legend (prior dynamic year range, recent, period average, this week, rug/dots) — TREND-01, TREND-02, TREND-03, Validated in Phase 8 (v1.2)
...
| Per-day trend row rendered as a split-violin (prior-25yr left vs recent-5yr right) instead of a dot-strip + single average line | Comparing two distributions per day surfaces the climate-shift signal an average line hides; realizes the v2-backlog visualization idea early | ✓ Good — Validated in Phase 8 (hand-rolled Gaussian KDE + Silverman bandwidth, shared pooled bandwidth, per-half n≥20 curve-vs-rug gate, shared Y-scale; legend copy finalized via the PD-10 reviewer round-trip) |
