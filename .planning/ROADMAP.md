...
- [x] 06-04-PLAN.md — Fix G-06-11: portal the InfoTooltip popover to document.body with fixed edge-aware positioning so it renders above the panels and within frame (Wave 1)
...
- [x] 07-02-PLAN.md — Always-visible Methodology disclosure panel (native `<details>`/`<summary>`, locked copy) mounted after TrendRow (EXPLAIN-03)
...
- [x] 08-01-PLAN.md — Statistics & data-shape core: hand-rolled Gaussian KDE + Silverman (`kde.ts`), the per-half `n_min=20` curve-vs-rug gate, and `computeTrendDay` → two-sample recent/prior return (TREND-01/02)
